...
class SourceCatalogueTest {
    ...
    void numerote_les_extraits_a_partir_de_un_dans_l_ordre_recu() {
        Absorption absorption = SourceCatalogue.empty().absorbe(List.of(extrait(RAPPORT, 0), extrait(RAPPORT, 1)));
    ...
    void rend_son_numero_d_origine_a_un_extrait_deja_vu() {
        SourceCatalogue premier = SourceCatalogue.empty()
                .absorbe(List.of(extrait(RAPPORT, 0), extrait(RAPPORT, 1)))
                .catalogue();
    ...
    void distingue_deux_extraits_par_leur_document_autant_que_par_leur_position() {
        Absorption absorption = SourceCatalogue.empty().absorbe(List.of(extrait(RAPPORT, 0), extrait(NOTES, 0)));
    ...
    void ne_rend_que_les_sources_reellement_citees_et_connues() {
        ...
        assertThat(catalogue.citees(List.of(3, 9, 1)))
                .extracting(Source::number)
                .containsExactly(3, 1);
    }
}
