 ...
 * the name Drive gives it and the format of the file it exports into, and it alone carries a
 * workspace type.
 */
public record DriveFile(
    ...
    public static DriveFile exported(
        ...
        return new DriveFile(id, name, format, 0, webViewLink, modifiedTime, workspaceType);
    }
    ...
    /**
     * What the document takes as its name, which is not what Drive calls the file: a native Doc
     * carries no extension, and {@code DocumentFormat} reads a format from a name. The Drive name
     * is the one a rejection tells, the owner having no other to recognise the file by.
     */
    public String documentName() {
        return workspaceType == null ? name : workspaceType.exportedName(name);
    }
