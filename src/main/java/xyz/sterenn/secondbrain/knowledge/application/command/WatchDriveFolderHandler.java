...
public class WatchDriveFolderHandler implements CommandHandler<WatchDriveFolder> {
    ...
    public void handle(WatchDriveFolder command) {
        ...
        DriveFolder folder = googleDriveFolders
                .folder(accessToken, command.folderId().trim())
                .orElseThrow(DriveFolderNotFoundException::new);

        refuseIfAlreadyCovered(connection, accessToken, folder);
     ...
     */
    private void refuseIfAlreadyCovered(DriveConnection connection, DriveAccessToken accessToken, DriveFolder folder) {
        ...
        for (String ancestor : googleDriveFolders.ancestors(accessToken, folder.id())) {
